...
## Summary
* Cleaned up visualizations / recreations from Washington Post with Albers projection, change in color, scales
* Created a function to plot individual visualizations
* Created two scripts: texas_load and texas_wind that read in the dataset and clean them from the raw data files
...
file_generators_plants <- 
  here::here("amy-kouch/c01-own/data/generators_plants.rds")

file_texas_load <- 
  here::here("amy-kouch/c01-own/data/texas_load_{year_texas}.rds")

file_texas_wind <- 
  here::here("amy-kouch/c01-own/data/wind_generation_{year_texas}.rds")
...
According to the [EIA](https://www.eia.gov/state/?sid=TX#tabs-1), "Texas leads the nation in wind-powered generation and produced one-fourth of all the U.S. wind powered electricity in 2017. Texas wind turbines have produced more electricity than both of the state's nuclear power plants since 2014." 
...
```{r echo = FALSE}
sum(texas_wind$wind_output_MW)/sum(texas_wind$load_MW)
...
The goal is to understand the potential of Texas wind and whether these goals are realistic.

In Texas, [ERCOT](http://www.ercot.com/about) is a utility provider that accounts for up to 90% of the electricity in Texas. We are using ERCOT load and generation data for this analysis. The [Advanced Energy Economy Institute](https://info.aee.net/hubfs/EPA/AEEI-Renewables-Grid-Integration-Case-Studies.pdf?t=1440089933677) has a report that Renewable Energy integration with ERCOT would be technically feasible. 

```{r echo = FALSE}
texas_wind %>%
  pivot_longer(
    cols = c(load_MW, wind_output_MW),
    names_to = "category",
    values_to = "MW"
  ) %>%
  ggplot(aes(date_time, MW, color = category)) +
  geom_line() + 
  labs(
    title = "Comparing Texas Wind Power to Overall Generation"
  )
...
texas_wind %>%
  ...
  summarise(
    ...
    max_wind = max(wind_output_MW)
  ) %>%
  ggplot(aes(hour)) +
  geom_point(aes(y = max_load)) +
  geom_line(aes(y = max_load)) +
  labs(
    title = "Maximum Load (MW) in 2018 per Hour"
  )
...
texas_wind %>%
  ...
  ggplot(aes(hour)) +
  geom_point(aes(y = max_wind)) +
  geom_line(aes(y = max_wind)) +
  labs(
    title = "Maximum Wind (MW) in 2018 per Hour"
  )
...
* Labeling the states
* Slightly-off data. For example, 375 coal plants versus 400 coal plants.
* Confirmation of results via article

## Challenge for Team

* Understanding wind in Texas - during what time of day is wind best? Worst? Which month has the highest wind output? 

## Next Steps

* Try to find more sources of information to confirm the results of this analysis
* Try to find discrepancies in why the results are not exact
* Determine how many MW of wind Texas will need in order to meet expected renewable mix of 50% wind -- "is this a reasonable expectation?"
